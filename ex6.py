import math

a = int(input('Digite um número: '))
print(f'\nO dobro de {a} vale {2*a}')
print(f'O triplo de {a} vale {3*a}')
print(f'A raiz quadrada de {a} vale {math.sqrt(a):.2f}')


#Aplicação do conceito do exercício, notas musicais e frequência
#print('Dado, que a frequência da nota musical dó, é de 132 Hz. E sua oitava superior, possui o dobro dessa frequência, qual é o valor da frequência da oitava superior?')